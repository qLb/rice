...
local function tile_group(cls, wa, orientation, fact, group)
    ...
    local useless_gap_inner = tonumber(beautiful.useless_gap_inner_width) or useless_gap
    if useless_gap_inner < 0 then useless_gap_inner = 0 end
    ...
    for c = group.first,group.last do
        ...
        then
            ...
            if (geom[y]+geom[height]) >= wa[height] - wa[x] then
                bottom = true
            end

            if (geom[x]+geom[width]) >= wa[width] - wa[y] then
                right = true
            end
            ...
            if top then
                if bottom then
                    ...
                    geom[y] = geom[y] + useless_gap
                    geom[height] = geom[height] - (useless_gap + useless_gap_inner)
                end
            elseif bottom then
                geom[y] = geom[y] + useless_gap_inner
                geom[height] = geom[height] - (useless_gap + useless_gap_inner)
            else
                geom[y] = geom[y] + useless_gap_inner
                geom[height] = geom[height] - (2 * useless_gap_inner)
            end
            ...
            if left then
                if right then
                    ...
                    geom[x] = geom[x] + useless_gap
                    geom[width] = geom[width] - (useless_gap + useless_gap_inner)
                end
            elseif right then
                geom[x] = geom[x] + useless_gap_inner
                geom[width] = geom[width] - (useless_gap + useless_gap_inner)
            else
                geom[x] = geom[x] + useless_gap_inner
                geom[width] = geom[width] - (2 * useless_gap_inner)
            end
...
-- @param screen The screen number to tile.
uselesstile.bottom = {}
uselesstile.bottom.name = "tilebottom"
